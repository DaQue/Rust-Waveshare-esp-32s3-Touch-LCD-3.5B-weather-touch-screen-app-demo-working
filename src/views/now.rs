...
pub fn draw(fb: &mut Framebuffer, state: &AppState) {
    ...
    let card_top = 36;
    let card_h = if state.orientation.is_portrait() {
        156
    } else {
        140
    };
    ...
    if let Some(cw) = &state.current_weather {
        ...
        if state.orientation.is_portrait() {
            ...
            Text::new(&temp_text, Point::new(106, card_top + 46), temp_style)
                .draw(fb)
                .ok();
            ...
            Text::new(&feels_text, Point::new(106, card_top + 72), feels_style)
                .draw(fb)
                .ok();
            Text::new(&cw.condition, Point::new(106, card_top + 94), cond_style)
                .draw(fb)
                .ok();
            ...
            Text::new(hint, Point::new(18, card_top + card_h - 14), hint_style)
                .draw(fb)
                .ok();
        } else {
            ...
            Text::new(&temp_text, Point::new(106, card_top + 34), temp_style)
                .draw(fb)
                .ok();
            let trend = outdoor_temp_trend(&state.outdoor_temp_history);
            ...
            Text::new(&feels_text, Point::new(106, card_top + 56), feels_style)
                .draw(fb)
                .ok();
            Text::new(&cw.condition, Point::new(106, card_top + 76), cond_style)
                .draw(fb)
                .ok();
            let wind_text = format!("Wind {:.0} mph", cw.wind_mph);
            Text::new(&wind_text, Point::new(106, card_top + 96), cond_style)
                .draw(fb)
                .ok();
            ...
            Text::new(
                hint,
                Point::new(CARD_MARGIN + 6, card_top + card_h - 13),
                hint_style,
            )
            .draw(fb)
            .ok();
            ...
            Text::new("Indoor", Point::new(ind_x, card_top + 22), ind_style_label)
                .draw(fb)
                .ok();
            ...
            if let Some(t) = state.indoor_temp {
                ...
                Text::new(&t_text, Point::new(ind_x, card_top + 47), ind_style_temp)
                    .draw(fb)
                    .ok();
            }
            if let Some(h) = state.indoor_humidity {
                let h_text = format!("{:.0}% RH", h);
                Text::new(&h_text, Point::new(ind_x, card_top + 79), ind_style_small)
                    .draw(fb)
                    .ok();
            }
            if let Some(p) = state.indoor_pressure {
                ...
                Text::new(&p_text, Point::new(ind_x, card_top + 95), ind_style_small)
                    .draw(fb)
                    .ok();
            }
    ...
    if state.orientation.is_portrait() {
        ...
        Text::new(
            "Indoor",
            Point::new(CARD_MARGIN + 8, ind_card_y + 18),
            ind_label,
        )
        .draw(fb)
        .ok();
        ...
        if let Some(t) = state.indoor_temp {
            ...
            let unit_c = if state.use_celsius { "C" } else { "F" };
            let t_text = format!("{:.1}°{}", t_disp, unit_c);
            Text::new(&t_text, Point::new(ind_x2, ind_card_y + 18), ind_label)
                .draw(fb)
                .ok();
        ...
        if let Some(h) = state.indoor_humidity {
            let h_text = format!("{:.0}% RH", h);
            Text::new(&h_text, Point::new(ind_x2, ind_card_y + 18), ind_small)
                .draw(fb)
                .ok();
        }
        ...
        if let Some(p) = state.indoor_pressure {
            ...
            Text::new(
                &p_text,
                Point::new(CARD_MARGIN + 8, ind_card_y + 48),
                ind_small,
            )
            .draw(fb)
            .ok();
        ...
        if state.indoor_pressure.is_some() {
            if let Some((delta, label)) = state.pressure_history.pressure_trend() {
                ...
                Text::new(
                    &trend_text,
                    Point::new(CARD_MARGIN + 100, ind_card_y + 48),
                    ind_small,
                )
                .draw(fb)
                .ok();
        ...
        if let Some(fc) = &state.forecast {
            ...
            for (i, row) in fc.rows.iter().take(rows as usize).enumerate() {
                ...
                Text::new(&row.title, Point::new(CARD_MARGIN + 52, ry - 5), day_style)
                    .draw(fb)
                    .ok();
                // Condition: PROFONT_10 below day name
                Text::new(
                    &row.condition,
                    Point::new(CARD_MARGIN + 52, ry + 10),
                    cond_style,
                )
                .draw(fb)
                .ok();
        ...
        if let Some(fc) = &state.forecast {
            ...
            Text::new(
                "Forecast >",
                Point::new(CARD_MARGIN + 8, fc_top + 14),
                fc_label_style,
            )
            .draw(fb)
            .ok();
...
fn draw_alert_overlay(fb: &mut Framebuffer, state: &AppState, screen_w: i32, screen_h: i32) {
    ...
    let y = if state.orientation.is_portrait() {
        224
    } else {
        190
    };
    ...
    let mut row_y = y + 44;
    for line in crate::layout::word_wrap(&alert.headline, max_chars)
        .iter()
        .take(2)
    {
        if row_y + line_h > bottom_reserve {
            break;
        }
        Text::new(line, Point::new(CARD_MARGIN + 8, row_y), body_style)
            .draw(fb)
            .ok();
        row_y += line_h;
    }
    ...
    for raw in alert.description.lines() {
        ...
        if content_chars <= char_limit {
            Text::new("- ", Point::new(CARD_MARGIN + 8, row_y), dim_style)
                .draw(fb)
                .ok();
            Text::new(content, Point::new(CARD_MARGIN + 8 + 12, row_y), dim_style)
                .draw(fb)
                .ok();
        } else {
            let end = content
            ...
            Text::new(
                &content[..end],
                Point::new(CARD_MARGIN + 8 + 12, row_y),
                dim_style,
            )
            .draw(fb)
            .ok();
            Text::new(
                "...",
                Point::new(CARD_MARGIN + 8 + 12 + (char_limit as i32 - 3) * 6, row_y),
                dim_style,
            )
            .draw(fb)
            .ok();
    ...
    Text::new(&exp_text, Point::new(CARD_MARGIN + 8, y + h - 8), dim_style)
        .draw(fb)
        .ok();
    Text::with_alignment(
        "(tap to close)",
