...
pub fn draw(fb: &mut Framebuffer, state: &AppState) {
    ...
    if state.indoor_temp.is_none() && state.indoor_humidity.is_none() {
        ...
        Text::new(
            "BME280 sensor unavailable",
            Point::new(14, reading_y),
            warn_style,
        )
        .draw(fb)
        .ok();
    ...
    if let Some(temp) = state.indoor_temp {
        ...
        let x = 20;
        let y = reading_y;
        Text::new(&t, Point::new(x, y), primary_style).draw(fb).ok();
    ...
    Text::new(
        "-- Humidity",
        Point::new(graph_x + 138, legend_y),
        legend_style2,
    )
    .draw(fb)
    .ok();
...
fn draw_graph_data(
    ...
    if temp_len > 0 {
        ...
        Text::with_alignment(
            &format!("{:.0}", max_v),
            Point::new(graph_x - 4, graph_y + 10),
            axis_style,
            Alignment::Right,
        )
        .draw(fb)
        ...
        Text::with_alignment(
            &format!("{:.0}", min_v),
            Point::new(graph_x - 4, graph_y + graph_h - 4),
            axis_style,
            Alignment::Right,
        )
        .draw(fb)
    ...
    Text::new(
        left_label,
        Point::new(graph_x, graph_y + graph_h + 14),
        axis_style,
    )
    .draw(fb)
    .ok();
