...
impl PressureHistory {
    ...
    fn iter_short(&self) -> impl Iterator<Item = &PressureSample> {
        let start = if self.short_count < SHORT_CAP {
            0
        } else {
            self.short_idx
        };
    ...
    fn iter_long(&self) -> impl Iterator<Item = &PressureSample> {
        let start = if self.long_count < LONG_CAP {
            0
        } else {
            self.long_idx
        };
    ...
    fn extract_series_short(
        &self,
        f: impl Fn(&PressureSample) -> Option<f32>,
    ) -> Vec<(usize, f32)> {
        let mut out = Vec::new();
    ...
    pub fn short_bme_series(&self) -> Vec<(usize, f32)> {
        self.extract_series_short(|s| s.bme_hpa)
    }
    pub fn short_owm_series(&self) -> Vec<(usize, f32)> {
        self.extract_series_short(|s| s.owm_hpa)
    }
    pub fn long_bme_series(&self) -> Vec<(usize, f32)> {
        self.extract_series_long(|s| s.bme_hpa)
    }
    pub fn long_owm_series(&self) -> Vec<(usize, f32)> {
        self.extract_series_long(|s| s.owm_hpa)
    }
    ...
    fn min_max_short(
        &self,
        extract: impl Fn(&PressureSample) -> Option<f32>,
    ) -> Option<(f32, f32)> {
    ...
    pub fn short_bme_min_max(&self) -> Option<(f32, f32)> {
        self.min_max_short(|s| s.bme_hpa)
    }
    pub fn short_owm_min_max(&self) -> Option<(f32, f32)> {
        self.min_max_short(|s| s.owm_hpa)
    }
    pub fn long_bme_min_max(&self) -> Option<(f32, f32)> {
        self.min_max_long(|s| s.bme_hpa)
    }
    pub fn long_owm_min_max(&self) -> Option<(f32, f32)> {
        self.min_max_long(|s| s.owm_hpa)
    }
    ...
    pub fn pressure_trend(&self) -> Option<(f32, &'static str)> {
        if self.long_count >= 61 {
            if let Some(d) = self.bme_delta_n_long(60) {
                return Some((d, "3h"));
            }
        }
        if self.long_count >= 21 {
            if let Some(d) = self.bme_delta_n_long(20) {
                return Some((d, "1h"));
            }
        }
        if self.short_count >= 121 {
            if let Some(d) = self.bme_delta_n_short(120) {
                return Some((d, "30m"));
            }
        }
        None
    ...
    fn bme_delta_n_long(&self, n: usize) -> Option<f32> {
        ...
        let newest = self.long[(start + self.long_count - 1) % LONG_CAP]
            .bme_hpa
            .filter(|v| v.is_finite());
        let older = self.long[(start + self.long_count - 1 - n) % LONG_CAP]
            .bme_hpa
            .filter(|v| v.is_finite());
        match (older, newest) {
    ...
    fn bme_delta_n_short(&self, n: usize) -> Option<f32> {
        ...
        let newest = self.short[(start + self.short_count - 1) % SHORT_CAP]
            .bme_hpa
            .filter(|v| v.is_finite());
        let older = self.short[(start + self.short_count - 1 - n) % SHORT_CAP]
            .bme_hpa
            .filter(|v| v.is_finite());
        match (older, newest) {
            (Some(o), Some(n)) => Some(n - o),
    ...
    pub fn load_from_bytes(&mut self, bytes: &[u8]) {
        ...
        // Read short ring
        self.short_count =
            (u64::from_le_bytes(bytes[off..off + 8].try_into().unwrap()) as usize).min(SHORT_CAP);
        off += 8;
        self.short_idx =
            (u64::from_le_bytes(bytes[off..off + 8].try_into().unwrap()) as usize) % SHORT_CAP;
        off += 8;
        ...
        // Read long ring
        self.long_count =
            (u64::from_le_bytes(bytes[off..off + 8].try_into().unwrap()) as usize).min(LONG_CAP);
        off += 8;
        self.long_idx =
            (u64::from_le_bytes(bytes[off..off + 8].try_into().unwrap()) as usize) % LONG_CAP;
        off += 8;
        for i in 0..LONG_CAP {
            let bme = f32::from_le_bytes(bytes[off..off + 4].try_into().unwrap());
            off += 4;
